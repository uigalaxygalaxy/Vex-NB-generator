            ...
            if (Math.abs(element.getLeft() - ToCCoords.left) < ToCCoords.tolerance.left && Math.abs(element.getTop() - ToCCoords.top) < ToCCoords.tolerance.top) {
                if (element.asTable) {
                    ...
                    for (let j = 0; j < ToCDimensions.rows; j++) {
                        ...
                        if (entry) {
                            table.getCell(j + 1, pageNumberColumn).getText().setText(entry.page);
                            ...
                            table.getCell(j + 1, titleColumn).getText().setText(entry.title);
                            table.getCell(j + 1, titleColumn).getText().getTextStyle().setLinkUrl(`#slide=id.${entry.id}`);
                            table.getCell(j + 1, iterationColumn).getText().setText(entry.iteration);
                            table.getCell(j + 1, dateColumn).getText().setText(entry.date);
                        }
